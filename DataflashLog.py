...
class BinaryFormat(ctypes.LittleEndianStructure):
    ...
    def to_class(self):
        members = dict(
            ...
            MSG=self.type,
            SIZE=self.length,
            labels=self.labels.decode().split(",") if self.labels else [],
            _pack_=True)

        fieldtypes = [i for i in self.types.decode()]
        fieldlabels = self.labels.decode().split(",")
        if self.labels and (len(fieldtypes) != len(fieldlabels)):
            print("Broken FMT message for {} .. ignoring".format(
        ...
        # field access
        for (label, _type) in zip(fieldlabels, fieldtypes):
            def createproperty(name, form):
                # extra scope for variable sanity
                # scaling via _NAME and def NAME(self): return self._NAME / SCALE
                propertyname = name
                attributename = '_' + name
                scale = BinaryFormat.FIELD_SCALE.get(form, None)
                p = property(lambda x: getattr(x, attributename))
                ...
                try:
                    fields.append(
                        (attributename, BinaryFormat.FIELD_FORMAT[form]))
                except KeyError:
                    print('ERROR: Failed to add FMT type: {}, with format: {}'.format(
                        attributename, form))
                    raise
            createproperty(label, _type)
        ...
        # finally, create the class
        cls = type(
            'Log__{:s}'.format(self.name.decode()),
            (ctypes.LittleEndianStructure,),
            members
        )

        if ctypes.sizeof(cls) != cls.SIZE:
            print("size mismatch for {} expected {} got {}".format(
                cls, ctypes.sizeof(cls), cls.SIZE), file=sys.stderr)
#            for i in cls.labels:
...
class Channel(object):
    ...
    def getSegment(self, startLine, endLine):
        ...
        segment = Channel()
        segment.dictData = {
            k: v for k, v in self.dictData.iteritems() if k >= startLine and k <= endLine}
        return segment
    ...
    def getInterpolatedValue(self, lineNumber):
        (prevValue, prevValueLine) = self.getNearestValue(
            lineNumber, lookForwards=False)
        (nextValue, nextValueLine) = self.getNearestValue(
            lineNumber, lookForwards=True)
        if prevValueLine == nextValueLine:
            return prevValue
        weight = (lineNumber-prevValueLine) / \
            float(nextValueLine-prevValueLine)
...
class LogIterator:
    ...
    def jump(self, lineNumber):
        ...
        for lineLabel in self.iterators.keys():
            dataLabel = self.logdata.formats[lineLabel].labels[0]
            (value, lineNumber) = self.logdata.channels[lineLabel][dataLabel].getNearestValue(
                self.currentLine)
            self.iterators[lineLabel] = (
                self.logdata.channels[lineLabel][dataLabel].getIndexOf(lineNumber), lineNumber)
...
class DataflashLogHelper:
    ...
    def getTimeAtLine(logdata, lineNumber):
        ...
        sys.stderr.write("didn't find GPS data for " +
                         str(lineNumber) + " - using maxtime\n")
    ...
    def findLoiterChunks(logdata, minLengthSeconds=0, noRCInputs=True):
        ...
        od = collections.OrderedDict(
            sorted(logdata.modeChanges.items(), key=lambda t: t[0]))
        chunks = []
        for i in range(len(od.keys())):
            if od.values()[i][0] == "LOITER":
                ...
                chunkTimeSeconds = (DataflashLogHelper.getTimeAtLine(
                    logdata, endLine)-DataflashLogHelper.getTimeAtLine(logdata, startLine)+1) / 1000.0
...
class DataflashLog(object):
    ...
    def __init__(self, logfile=None, format="auto", ignoreBadlines=False):
        ...
        self.firmwareHash = ""
        self.freeRAM = 0
        # APM 1, APM 2, PX4, MPNG, etc What is VRBrain? BeagleBone, etc? Needs more testing
        self.hardwareType = ""
    ...
    def read(self, logfile, format="auto", ignoreBadlines=False):
        ...
        if format == 'bin':
            ...
            raise ValueError(
                "Unknown log format for {}: {}".format(self.filename, format))
    ...
    def handleModeChange(self, lineNumber, e):
        if self.vehicleType == VehicleType.Copter:
            ...
            try:
                if hasattr(e, 'ThrCrs'):
                    self.modeChanges[lineNumber] = (
                        modes[int(e.Mode)], e.ThrCrs)
                else:
                    # assume it has ModeNum:
                    self.modeChanges[lineNumber] = (
                        modes[int(e.Mode)], e.ModeNum)
            ...
            raise Exception("Unknown log type for MODE line vehicletype=({}) line=({})".format(
                self.vehicleTypeString, repr(e)))
            ...
            try:
                ...
                if len(tokens) == 1:
                    ...
                    if line == "":
                        pass
                    # log index
                    elif len(tokens2) == 1 and tokens2[0].isdigit():
                        pass
                    ...
                    # e.g. ArduCopter V3.1 (5c6503e2)
                    elif (len(tokens2) == 2 or len(tokens2) == 3) and tokens2[1][0].lower() == "v":
                        try:
                        ...
                        errorMsg = "Error parsing line %d of log file: %s" % (
                            lineNumber, self.filename)
                        if ignoreBadlines:
                ...
                if not ignoreBadlines:
                    raise Exception("Error parsing line %d of log file %s - %s" %
                                    (lineNumber, self.filename, e.args[0]))
        ...
        while len(data) > offset + ctypes.sizeof(logheader):
            ...
            if not (h.head1 == 0xa3 and h.head2 == 0x95):
                if ignoreBadlines == False:
                    ...
                    if h.head1 == 0xff and h.head2 == 0xff and h.msgid == 0xff:
                        print("Assuming EOF due to dataflash block tail filled with \\xff... (offset={off})".format(
                            off=offset), file=sys.stderr)
                        break
                    offset += 1
                    continue
            ...
            if h.msgid in self._formats:
                ...
                try:
                    ...
                    print("data:{} offset:{} size:{} sizeof:{} sum:{}".format(
